module EnrollmentShopping
  class EnrollmentBuilder
    ...
    def build_new_waiver_enrollment(is_qle: false, is_dual_oe: false, optional_effective_on: nil, waiver_reason: nil)
      enrollment = build_common_enrollment_information("employer_sponsored")
      enrollment.waiver_reason = waiver_reason
      benefit_package = nil
      ...
      if is_qle && enrollment.family.is_under_special_enrollment_period?
        if optional_effective_on.present?
          ...
          if possible_benefit_package
            ...
            # They are in a sep, but there is NO benefit package available then
            # Maybe they weren't hired yet
            effective_date = earliest_eligible_date_for_shop(@employee_role, is_dual_oe: is_dual_oe)
            enrollment.effective_on = effective_date
            benefit_package = benefit_package_for_date(@employee_role, effective_date)
          end
        end
    ...
    def build_new_enrollment(family_member_ids: [], is_qle: false, is_dual_oe: false, optional_effective_on: nil)
      enrollment = build_common_enrollment_information("employer_sponsored")
      benefit_package = nil
    ...
    def earliest_eligible_date_for_shop(employee_role, is_dual_oe: false)
      employee_role.census_employee.coverage_effective_on(is_dual_oe: is_dual_oe)
    end
  end
end
