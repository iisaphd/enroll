...
module Importers::Transcripts
  ...
  class EnrollmentTranscript
    ...
    def process
      ...
      if @transcript[:source_is_new] && valid_new_request?
      ...
      begin
        ...
        if @enrollment.present? && !@transcript[:source_is_new] # && @enrollment.updated_at <= Date.new(2016,11,9)
          if @canceled
            @enrollment.invalidate_enrollment! if @enrollment.may_invalidate_enrollment?
          elsif @other_enrollment.terminated_on.present?
            if HbxEnrollment::ENROLLED_STATUSES.include?(@enrollment.aasm_state) && @enrollment.may_terminate_coverage?
    ...
    def create_new_enrollment
      ...
      begin
        ...
        employee_role = matched_person.employee_roles.select {|e_role| e_role.employer_profile == employer_profile}.last
        ...
        employee_role ||= role
        hbx_enrollment = build_enrollment(family, employee_role, employer_profile)
        hbx_enrollment.invalidate_enrollment! if @canceled && hbx_enrollment.may_invalidate_enrollment?
    ...
    def find_census_employee(matched_person, employee_role, employer_profile)
      if employee_role.present?
        ...
        census_employees = CensusEmployee.matchable(matched_person.ssn, matched_person.dob).to_a + CensusEmployee.unclaimed_matchable(matched_person.ssn, matched_person.dob).to_a + CensusEmployee.terminated_matchable(matched_person.ssn, matched_person.dob).to_a
    ...
    def build_enrollment(family, employee_role, employer_profile)
      ...
      product = @other_enrollment.product
    ...
    def build_consumer_role(matched_person , member)
      matched_person.build_consumer_role(ssn: matched_person.ssn,
    ...
    def build_hbx_enrollment_members(family, other_enrollment, hbx_enrollment)
      other_enrollment.hbx_enrollment_members.each do |member|
        matched_people = match_person_instance(member.family_member.person)
        ...
        if @market == 'individual'
          if matched_person.consumer_role.blank?
            consumer_role = build_consumer_role(matched_person, member)
