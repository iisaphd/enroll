...
And(/(.*) has active coverage in coverage enrolled state/) do |named_person|
  ...
  person_rec = Person.where(first_name: /#{person[:first_name]}/i, last_name: /#{person[:last_name]}/i).first ||
               FactoryBot.create(:person, :with_family, first_name: ce.first_name, last_name: ce.last_name, dob: ce.dob, ssn: ce.ssn)
  employee_role = FactoryBot.create(:employee_role, person: person_rec, benefit_sponsors_employer_profile_id: ce.benefit_sponsorship.profile.id, census_employee_id: ce.id)
  ce.update_attributes(employee_role_id: employee_role.id)
