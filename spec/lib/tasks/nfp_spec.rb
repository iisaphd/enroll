...
RSpec.describe 'upload the invoice to s3', :type => :task, dbclean: :after_each do
  ...
  let!(:organization)        { FactoryBot.create(:benefit_sponsors_organizations_general_organization, :with_aca_shop_cca_employer_profile, site: site, hbx_id: "211045") }
  ...
  let!(:benefit_application) {
    application = FactoryBot.create(:benefit_sponsors_benefit_application, :with_benefit_sponsor_catalog, benefit_sponsorship: benefit_sponsorship, aasm_state: :active, effective_period: effective_period)
